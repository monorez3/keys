...
## `dev` — уязвимость, домен или адрес
...
# domain github.com · MarkMonitor Inc. · истекает 2026-10-09 · 140.82.121.3 · копия 2026-08-30

k.dev.text("8.8.8.8")
# ip 8.8.8.8 · Ashburn United States · Google LLC
```

Что именно вы дали, ключ понимает сам — номер уязвимости, домен и адрес друг
на друга не похожи. Три вещи, которые разработчик ищет чаще всего и каждый раз
открывает три разных сайта.

Источники открытые и без ключей: CIRCL (зеркало базы CVE), RDAP вместо
устаревшего whois, DNS через Cloudflare, Веб-архив, ip-api.

## `crypto` — цена монеты

```python
k.crypto.text("биткоин")            # Bitcoin · 78 293 USD · за сутки +0.08% · место 1
k.crypto.text("курс тона в шекелях")  # The Open Network · 4 ILS · за сутки -1.81%
k.crypto("ETH", vs="usd")
```

Отдельно от обычных курсов намеренно: у валюты курс — одно число от
центробанка на день, у монеты — цена сразу в нескольких валютах, движение за
сутки и капитализация, и всё это меняется ежеминутно.
...
### Аргументы делают ровно то, что просят

`only=` берёт сколько угодно полей — приложению обычно нужны два-три из
двадцати, а не всё подряд:
...
Одно поле остаётся голым значением — его можно сразу класть в переменную.
Опечатка в любом из имён отвечает отказом со списком настоящих полей.

У `answer` перечисленные поля — это ещё и указание, кого спрашивать:

```
/answer/Хайфа?only=weather,osm
→ Хайфа: 30.4°C, ветер 20.5 м/с · Хайфа, נפת חיפה, Хайфский округ, Израиль

/answer/Хайфа?sources=osm,wikidata
→ osm: Хайфа, נפת חיפה… · wikidata: Хайфа — город на севере Израиля…
```

Назвали несколько источников — увидите всех, кто ответил, а не одного
победителя: раз перечислили, значит хотите сравнить.

`lang=` выбирает язык источника. Спрашивать английский источник русским
словом не нужно: английская Википедия по слову «Хайфа» находит не город, а
однофамильца-футболиста, поэтому такой ответ отбрасывается и ключ честно
молчит.

```
/answer/Haifa?sources=wiki&lang=en   → Haifa is the third-largest city…
/answer/Хайфа?sources=wiki           → Ха́йфа — третий по величине город…
```
