...
### Аргументы отдельных ключей

| ключ | аргумент | что делает |
|---|---|---|
| `answer` | `sources` | какие источники спрашивать; несколько — через запятую, пусто — ключ выберет сам |
| `answer` | `lang` | язык источника: `ru` или `en` |
| `crypto` | `vs` | в каких валютах показать цену, через запятую |

```python
k.answer('Хайфа', sources='osm,wikidata')   # оба ответа, а не один
k.answer('Haifa', sources='wiki', lang='en')
k.crypto('bitcoin', vs='usd,ils')

k.alive('@durov', only='title,members_count')   # несколько полей сразу
# 'Pavel Durov · 11005185'
```

Спрашивайте источник на его языке: `lang='en'` со словом «Хайфа» английская
Википедия честно не найдёт — и промолчит, а не выдумает.
