...
async def _на_языке(запрос: str, язык: str, ctx) -> str:
    """Название на языке источника.

    Если человек прямо просит lang=en, а спрашивает по-русски, английская
    Википедия по слову «Хайфа» не найдёт ничего — и аргумент выглядит
    сломанным, хотя сделал ровно что просили. Карта умеет отдавать названия
    на нужном языке, ей и переводим. Явный аргумент обязан работать.
    """
    if язык == "ru" or not КИРИЛЛИЦА.search(запрос):
        return запрос
    места = await _на_карте(запрос, язык, ctx)
    if not места:
        return запрос
    название = (места[0].get("display_name") or "").split(",")[0].strip()
    return название or запрос
...
async def _wiki(в: str, язык: str, ctx) -> tuple[str, str]:
    найдено = json.loads((await ctx.fetch(
        f"https://{язык}.wikipedia.org/w/api.php?" + urlencode({
            ...
            "srsearch": await _на_языке(без_темы(суть(в)), язык, ctx),
...
async def run(params: dict, ctx) -> dict:
    ...
    # Проверка «про то ли это» сравнивает слова вопроса и ответа, а через
    # языки так нельзя: на «Хайфа» английская Википедия отвечает «Haifa is
    # the third-largest city…», общих слов ноль, и честный ответ выбрасывался.
    # Если язык источника задан явно и не совпадает с языком вопроса —
    # сравнивать нечего, доверяем источнику.
    через_языки = язык != язык_вопроса(вопрос)
    ответы = {
        имя: (текст if через_языки or имя not in ОБЩИЕ or похоже(вопрос, текст) else "")
