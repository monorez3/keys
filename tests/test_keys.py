...
# --- второй ключ: кто кому сколько должен ----------------------------------- #

# оба обработчика называются handler — обычный import вернул бы первый,
# поэтому грузим по пути, как это делает реестр
import importlib.util  # noqa: E402

_спец = importlib.util.spec_from_file_location(
    "handler_settle", ROOT / "keys" / "settle" / "handler.py"
)
settle = importlib.util.module_from_spec(_спец)
_спец.loader.exec_module(settle)


def test_баланс_сходится_в_ноль():
    """Сумма всех балансов обязана быть нулём — иначе деньги родились из воздуха."""
    записи = settle.разобрать("аня:1200:аня,боря,вика; боря:600:боря,вика")
    assert abs(sum(settle.балансы(записи).values())) < 0.01


def test_план_гасит_все_долги():
    записи = settle.разобрать("а:1000:а,б,в,г,д; б:500:а,б,в,г,д; в:250:а,б,в,г,д")
    баланс = settle.балансы(записи)
    остаток = dict(баланс)
    for кто, кому, сколько in settle.план(баланс):
        остаток[кто] += сколько
        остаток[кому] -= сколько
    assert all(abs(v) < 0.01 for v in остаток.values()), "после плана долгов быть не должно"


def test_переводов_не_больше_чем_участников_минус_один():
    """Обещание из описания. Жадный метод даёт эту границу всегда."""
    для_проверки = [
        "а:1000:а,б,в,г,д; б:500:а,б,в,г,д; в:250:а,б,в,г,д",
        "а:10; б:20; в:30; г:40",
        "а:100:а,б; б:300:а,б; в:50:в",
    ]
    for строка in для_проверки:
        r = settle.посчитать(строка)
        assert r["transfers_count"] <= r["people"] - 1, строка


def test_когда_все_квиты_переводов_нет():
    r = settle.посчитать("аня:300:аня,боря; боря:300:аня,боря")
    assert r["transfers_count"] == 0
    assert "никто никому" in r["transfers"]


def test_участники_подставляются_сами_если_не_указаны():
    явно = settle.посчитать("аня:900:аня,боря,вика; боря:300:аня,боря,вика")
    неявно = settle.посчитать("аня:900; боря:300; вика:0")
    assert явно["balances"] == неявно["balances"]


def test_мусор_объясняет_себя():
    для_отказа = {
        "": "пустой",
        "аня": "не хватает суммы",
        "аня:двести": "не число",
        "аня:-50": "отрицательная",
        "аня:100:боря:лишнее": "слишком много двоеточий",
        ":100": "не указано, кто платил",
    }
    for строка, ожидание in для_отказа.items():
        with pytest.raises(ValueError) as exc:
            settle.разобрать(строка)
        assert ожидание in str(exc.value), f"для {строка!r} ждали «{ожидание}», получили {exc.value}"


def test_одни_и_те_же_расходы_это_один_ключ_кэша():
    """Пробелы и порядок записей не должны плодить лишние записи в кэше."""
    формы = [
        "аня:1200:аня,боря,вика; боря:600:боря,вика",
        "  боря : 600 : вика,боря ;аня:1200:вика,аня,боря  ",
        "аня:1200:боря,вика,аня\nборя:600:вика,боря",
    ]
    каноны = {settle.canonical({"expenses": ф})["expenses"] for ф in формы}
    assert len(каноны) == 1


def test_оба_ключа_на_месте_и_описаны():
    ключи = discover(ROOT / "keys")
    assert set(ключи) == {"alive", "settle"}
    for ключ in ключи.values():
        поля = {r.name for r in ключ.manifest.returns}
        assert ключ.manifest.short["field"] in поля, "поле для короткого ответа не описано"
        assert ключ.manifest.primary_param().name in {p.name for p in ключ.manifest.params}
...
def test_страницы_открываются():
    for путь in ["/", "/client", "/keys", "/llms.txt", "/openapi.json", "/health",
                 "/k/alive/docs", "/k/settle/docs", "/public-token", "/sdk/simple/"]:
...
def test_счётный_ключ_работает_без_сети():
    """settle ничего не качает, поэтому его можно проверить прямо тут."""
    ответ = клиент.get("/settle/аня:1200:аня,боря; боря:600:аня,боря")
    assert ответ.status_code == 200
    assert "перевод" in ответ.text

    полный = клиент.get("/k/settle", params={"expenses": "а:100:а,б; б:300:а,б"})
    assert полный.status_code == 200
    тело = полный.json()
    assert тело["ok"] is True and тело["transfers_count"] == 1


def test_кривой_вход_объясняет_себя_а_не_падает():
    ответ = клиент.get("/settle/аня:двести")
    assert ответ.status_code == 400
    assert "не число" in ответ.text
