...
# --- ключ «ответ»: разбор вопроса и выбор источников ------------------------ #
#
# Сети тут нет: проверяется то, что решает ключ ДО похода наружу. Именно в
# этом месте и жили все находки живого прогона — источник искал не то, что
# нужно, и честно отвечал «ничего не нашёл».

_спец_отв = importlib.util.spec_from_file_location(
    "handler_answer", ROOT / "keys" / "answer" / "handler.py"
)
отв = importlib.util.module_from_spec(_спец_отв)
_спец_отв.loader.exec_module(отв)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("что такое кватернион", "кватернион"),
        ("где находится Хайфа", "Хайфа"),
        ("who is Pavel Durov", "Pavel Durov"),
    ],
)
def test_вопросительные_слова_снимаются(вопрос, ожидаем):
    """Источники ищут сущность, а не фразу: на полный вопрос они отвечают пустотой."""
    assert отв.суть(вопрос) == ожидаем


@pytest.mark.parametrize(
    "вопрос,ожидаем",
    [
        ("погода в Хайфе", "Хайфе"),
        ("пакет monokeys", "monokeys"),
        ("репозиторий telegram", "telegram"),
        ("книга Мастер и Маргарита", "Мастер и Маргарита"),
    ],
)
def test_слово_темы_снимается(вопрос, ожидаем):
    """Карта не знает места «погода в Хайфе», реестр — пакета «пакет monokeys»."""
    assert отв.без_темы(вопрос) == ожидаем


def test_ударения_не_мешают_сравнению():
    """В Википедии пишут «Па́вел» — для глаза то же слово, для строк разные."""
    assert отв.без_ударений("Па́вел Ду́ров") == "Павел Дуров"
    assert отв.похоже("кто такой Павел Дуров", "Па́вел Вале́рьевич Ду́ров — предприниматель")


def test_ответ_не_по_теме_отсеивается():
    """Википедия всегда что-нибудь находит: на «погоду в Хайфе» она выдавала
    театрального режиссёра, и это уходило человеку как лучший ответ."""
    assert отв.похоже("погода в Хайфе", "Иосиф Райхельгауз — театральный режиссёр") is False
    assert отв.похоже("курс доллара к шекелю", "Египетский фунт — денежная единица Египта") is False


def test_ключ_сам_выбирает_источники():
    общая = set(отв.ВСЕГДА)
    assert set(отв.подобрать("кто такой Дуров")) == общая
    assert "weather" in отв.подобрать("погода в Хайфе")
    assert "rates" in отв.подобрать("курс доллара к шекелю")
    assert "osm" in отв.подобрать("где находится Хайфа")
    assert "pypi" in отв.подобрать("пакет monokeys")
    assert "github" in отв.подобрать("репозиторий telegram")
    assert "books" in отв.подобрать("книга Мастер и Маргарита")
    for вопрос in ["кто такой Дуров", "погода в Хайфе", "пакет monokeys"]:
        assert общая <= set(отв.подобрать(вопрос)), "общую справку спрашиваем всегда"


def test_источники_можно_задать_руками():
    assert отв.выбрать_источники("wiki,osm", "что угодно") == ["wiki", "osm"]
    assert отв.выбрать_источники("all", "что угодно") == отв.ВСЕ
    with pytest.raises(ValueError, match="источника"):
        отв.выбрать_источники("гугл", "что угодно")


def test_порядок_источников_не_плодит_записи_кэша():
    первый = отв.canonical({"q": "Хайфа", "sources": "osm,wiki"})
    второй = отв.canonical({"q": " Хайфа ", "sources": "wiki, osm"})
    assert первый == второй


def test_мусор_на_входе_объясняет_себя():
    for плохо, ждём in [({"q": ""}, "пустой"), ({"q": "а" * 400}, "длиннее"),
                        ({"q": "х", "lang": "де"}, "не поддержан")]:
        with pytest.raises(ValueError, match=ждём):
            отв.canonical(плохо)


def test_у_ключа_описаны_все_источники():
    """Источник появился в коде, а в манифест его не внесли — тогда его не
    вернуть через only= и не увидеть в документации."""
...
def test_валюты_и_суммы_разбираются():
    assert отв.валюты_из("100 долларов в шекели", ИЗВЕСТНЫЕ) == ["USD", "ILS"]
    assert отв.валюты_из("100 usd to eur", ИЗВЕСТНЫЕ) == ["USD", "EUR"]
    assert отв.сумма_из("100 долларов") == 100.0
    assert отв.сумма_из("курс доллара") == 1.0


def test_рубль_и_прочие_валюты_доступны():
    """Раньше курсы брались у ЕЦБ: тридцать валют и никакого рубля."""
    assert отв.валюты_из("курс рубля к доллару", ИЗВЕСТНЫЕ) == ["RUB", "USD"]
    assert отв.валюты_из("50000 рублей в тенге", ИЗВЕСТНЫЕ) == ["RUB", "KZT"]
    # «гривен» — не «гривн»: отсюда короткая основа, иначе склонение мимо
    assert отв.валюты_из("1000 гривен в рублях", ИЗВЕСТНЫЕ) == ["UAH", "RUB"]
    # код из трёх букв работает для всех 166, даже если словом мы его не знаем
    assert отв.валюты_из("100 UZS в KGS", ИЗВЕСТНЫЕ) == ["UZS", "KGS"]


def test_основы_валют_не_ловятся_внутри_обычных_слов():
    """«сумма» не должна становиться узбекским сумом, «драма» — армянским драмом."""
    assert отв.валюты_из("сколько будет сумма 100 долларов", ИЗВЕСТНЫЕ) == ["USD"]
    assert отв.валюты_из("что такое драма в театре", ИЗВЕСТНЫЕ) == []


def test_коды_валют_не_ловятся_в_английских_вопросах():
    """«who is…» и «what is the key» — не вопросы про курсы, хотя тоже
    состоят из трёхбуквенных слов."""
    for вопрос in ["who is Pavel Durov", "what is the key", "how are you"]:
        assert "rates" not in отв.подобрать(вопрос), вопрос
    for вопрос in ["100 UZS в KGS", "500 GEL to AMD"]:
        assert "rates" in отв.подобрать(вопрос), вопрос


def test_мелкий_курс_не_схлопывается_в_ноль():
    """«1 RUB = 0.01 USD» — это округление, а не ответ."""
    assert отв._красиво(0.0116) == "0.0116"
    assert отв._красиво(297.28) == "297.28"
    assert отв._красиво(100.0) == "100"
...
def test_чистка_кэша_убирает_оба_слоя(tmp_path):
    """Слой в памяти живёт отдельно от базы: стёр таблицу — а сервер ещё
    полчаса отдаёт старое. На этом я и попался вживую."""
    from runtime import Cache

    кэш = Cache(tmp_path / "c.db")
    кэш.put("к", {"ответ": 1}, ttl=3600)
    assert кэш.get("к") == {"ответ": 1}
    assert кэш.mem, "в памяти должно что-то лежать"
...
def test_чужой_язык_источника_остаётся_ошибкой_спросившего():
    """Английская Википедия по слову «Хайфа» отдаёт футболиста, а не город.
    Поэтому послаблений в проверке для чужого языка нет: молчание правильнее
    уверенно неверного ответа."""
    assert отв.похоже("Хайфа", "Haifa is the third-largest city in Israel") is False
    assert отв.похоже("Хайфа", "Abdoulaye Seck is a Senegalese footballer") is False
